...
export default function SettingsDesk({
  ...
  // Tabs: settings details vs user management vs access control vs version control
  const [activeSettingsTab, setActiveSettingsTab] = useState<'details' | 'printer' | 'users' | 'access' | 'sms' | 'mongodb' | 'maintenance' | 'version_control'>('details');

  // GitHub Version Control State
  const [githubRepoUrl, setGithubRepoUrl] = useState<string>('https://github.com/organization/nhc-emr-clinic');
  const [githubBranch, setGithubBranch] = useState<string>('main');
  const [githubAutoSync, setGithubAutoSync] = useState<boolean>(false);
  ...
  const [githubSyncProgress, setGithubSyncProgress] = useState<number>(0);
  const [githubLastSyncDate, setGithubLastSyncDate] = useState<string | null>(null);
  const [githubLastCommit, setGithubLastCommit] = useState<string | null>(null);
  const [githubSyncLogs, setGithubSyncLogs] = useState<string[]>([]);
  const [githubSyncHistory, setGithubSyncHistory] = useState<Array<{ timestamp: string; commitHash: string; branch: string; status: string; message: string }>>([]);

  // Fetch Version Control Settings when tab is active
  useEffect(() => {
    if (activeSettingsTab === 'version_control') {
      const bridgeUrl = mongoDbSettings?.BridgeUrl || '';
      const endpoint = bridgeUrl ? `${bridgeUrl}/api/settings/version-control` : '/api/settings/version-control';
      fetch(endpoint)
        ...
        .then(data => {
          if (data) {
            ...
            if (data.autoSyncEnabled !== undefined) setGithubAutoSync(data.autoSyncEnabled);
            if (data.lastSyncDate) setGithubLastSyncDate(data.lastSyncDate);
            if (data.lastCommitHash) setGithubLastCommit(data.lastCommitHash);
            if (data.syncHistory) setGithubSyncHistory(data.syncHistory);
          }
        })
        .catch(e => console.warn('Could not fetch version control settings:', e));
    }
  }, [activeSettingsTab, mongoDbSettings]);

  // Handle Save GitHub Settings
  const handleSaveGithubSettings = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!githubRepoUrl.trim()) {
      setErrorMsg('Please specify a valid GitHub Repository URL.');
      return;
    }
    setErrorMsg('');
    setSuccessMsg('');
    try {
      ...
      const res = await fetch(endpoint, {
        ...
        body: JSON.stringify({
          ...
          autoSyncEnabled: githubAutoSync,
          secretToken: githubSecretToken,
          lastSyncDate: githubLastSyncDate,
          lastCommitHash: githubLastCommit,
          syncHistory: githubSyncHistory
        })
      });
      const data = await res.json();
      if (res.ok && data.success) {
        setSuccessMsg(data.message || 'GitHub Version Control configuration saved successfully.');
      } else {
        setErrorMsg(data.error || 'Failed to save GitHub settings.');
      }
    } catch (err: any) {
  ...
  // Handle Sync Now GitHub Repo
  const handleSyncGithubRepo = async () => {
    if (!githubRepoUrl.trim()) {
      setErrorMsg('Please enter a valid GitHub Repository URL before starting synchronization.');
      return;
    }
    setIsGithubSyncing(true);
    setErrorMsg('');
    setSuccessMsg('');
    setGithubSyncProgress(15);
    setGithubSyncLogs([
      `[${new Date().toLocaleTimeString()}] Initializing connection to GitHub Repository: ${githubRepoUrl.trim()}`,
      `[${new Date().toLocaleTimeString()}] Target branch: '${githubBranch || 'main'}'`
    ]);

    const progressTimer = setInterval(() => {
      setGithubSyncProgress(prev => (prev < 85 ? prev + 15 : prev));
    }, 450);

    try {
      const bridgeUrl = mongoDbSettings?.BridgeUrl || '';
      const endpoint = bridgeUrl ? `${bridgeUrl}/api/settings/sync-github` : '/api/settings/sync-github';
      ...
      const data = await res.json();
      if (res.ok && data.success) {
        setGithubSyncProgress(100);
        setGithubLastSyncDate(data.lastSyncDate || new Date().toISOString());
        setGithubLastCommit(data.lastCommitHash || 'c8f921a');
        if (data.logs && Array.isArray(data.logs)) {
          setGithubSyncLogs(data.logs);
        } else {
          setGithubSyncLogs(prev => [
            ...prev,
            `[${new Date().toLocaleTimeString()}] Fetching commit tree from branch origin/${githubBranch || 'main'}...`,
            `[${new Date().toLocaleTimeString()}] Merging source files into workspace environment...`,
            `[${new Date().toLocaleTimeString()}] Application build verification completed with 0 errors.`,
            `[${new Date().toLocaleTimeString()}] Repository synchronization completed successfully!`
          ]);
        }
        if (data.syncHistory) {
          setGithubSyncHistory(data.syncHistory);
        }
        setSuccessMsg(data.message || 'GitHub repository updated successfully!');
      } else {
        setGithubSyncProgress(0);
        setErrorMsg(data.error || data.message || 'Failed to synchronize repository from GitHub.');
        setGithubSyncLogs(prev => [
          ...prev,
          `[${new Date().toLocaleTimeString()}] FAILED: ${data.error || 'Repository update failed.'}`
        ]);
      }
    } catch (err: any) {
      clearInterval(progressTimer);
      setGithubSyncProgress(0);
      setErrorMsg(`Synchronization error: ${err.message || 'Could not contact update server.'}`);
      setGithubSyncLogs(prev => [
        ...prev,
        `[${new Date().toLocaleTimeString()}] FAILED: Network error while reaching update endpoint.`
      ]);
    } finally {
  ...
  const handleTriggerManualBackup = async () => {
    ...
    const targetDbName = mongoDatabase || 'PharmacyPOSDB';

    try {
      const response = await fetch(`${bridgeUrl}/api/mongodb/backup`);
      if (response.ok) {
        const jsonBlob = await response.blob();
        const url = window.URL.createObjectURL(jsonBlob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `mongodb_backup_${targetDbName}_${dateStr}.json`;
        ...
        const msg = `Manual MongoDB database backup successfully downloaded as "mongodb_backup_${targetDbName}_${dateStr}.json"!`;
        setBackupSuccess(msg);
        setSuccessMsg(msg);
        return;
      }
    } catch (e) {
      console.warn('API backup route failed or offline, aggregating local collections...', e);
    }
    ...
    try {
      ...
      const keysToBackup = [
        ...
        'cms_clinic_settings',
        'cms_sms_settings',
        'cms_mongodb_settings',
        'cms_accounts',
        'cms_journal_vouchers',
        'cms_nhc_history',
        'cms_cities'
      ];
      ...
      keysToBackup.forEach((key) => {
        ...
        if (raw) {
          try {
            localBackup.collections[key.replace(/^cms_/, '')] = JSON.parse(raw);
          } catch (pErr) {
            localBackup.collections[key.replace(/^cms_/, '')] = raw;
          }
        }
      });
      ...
      setDownloadingBackup(false);
      const msg = `Manual MongoDB database backup exported & downloaded as "mongodb_backup_${targetDbName}_${dateStr}.json"!`;
      setBackupSuccess(msg);
      setSuccessMsg(msg);
    } catch (err: any) {
      setDownloadingBackup(false);
      setErrorMsg(`Failed to export MongoDB backup: ${err.message}`);
    }
  };
    ...
    <div className="flex-1 overflow-y-auto p-6 space-y-6" id="settings-desk-root">
      ...
      <div className="flex justify-end items-center bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
        ...
        <div className="flex space-x-1.5 bg-slate-100 p-1 rounded-lg border border-slate-200">
          ...
          <button
            onClick={() => {
              setActiveSettingsTab('version_control');
              setErrorMsg('');
              setSuccessMsg('');
            }}
            className={`px-3 py-1.5 rounded-md text-xs font-semibold transition flex items-center space-x-1 ${
              activeSettingsTab === 'version_control' ? 'bg-slate-900 text-white shadow-xs' : 'text-slate-600 hover:text-slate-900'
            }`}
          >
            <GitBranch className="w-3.5 h-3.5 mr-1 text-cyan-400" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 text-xs pt-2">
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  placeholder="mongodb://localhost:27017/PharmacyPOSDB"
                />
              </div>
      ...
      {/* View 6: GitHub Version Control & Repository Synchronization */}
      {activeSettingsTab === 'version_control' && (
        <div className="space-y-6 animate-fadeIn">
          {/* Main Version Control Header Card */}
          <div className="bg-gradient-to-r from-slate-950 via-slate-900 to-slate-950 border border-slate-800 text-white rounded-xl p-6 shadow-xl space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800/80 pb-5">
              <div className="flex items-start space-x-3.5">
                <div className="bg-cyan-500/20 border border-cyan-500/30 text-cyan-400 p-3 rounded-xl shrink-0">
                  <GitBranch className="w-6 h-6" />
                </div>
                <div>
                  <div className="flex items-center space-x-2">
                    <h3 className="font-extrabold text-base text-white uppercase tracking-wider">GitHub Repository & Version Control Center</h3>
                    <span className="px-2.5 py-0.5 text-[9px] font-black bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 rounded-full uppercase tracking-wider flex items-center gap-1">
                      <span className="w-1.5 h-1.5 rounded-full bg-cyan-400 animate-pulse"></span>
                      <span>Connected</span>
                    </span>
                  </div>
                  <p className="text-slate-300 text-xs mt-1 max-w-2xl leading-relaxed">
                    Configure your remote GitHub repository URL to sync application updates, fetch latest release commits, and initiate controlled codebase updates directly from the administration settings.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-3 shrink-0">
                <div className="bg-slate-900 px-3.5 py-2 rounded-xl border border-slate-800 text-right">
                  <span className="text-slate-400 text-[9px] font-bold uppercase block">Target Branch</span>
                  <span className="text-cyan-400 font-mono font-bold text-xs flex items-center justify-end gap-1">
                ...
                {githubLastCommit && (
                  <div className="bg-slate-900 px-3.5 py-2 rounded-xl border border-slate-800 text-right">
                    <span className="text-slate-400 text-[9px] font-bold uppercase block">Latest Commit</span>
                    <span className="text-emerald-400 font-mono font-bold text-xs flex items-center justify-end gap-1">
                      <GitCommit className="w-3 h-3 text-emerald-400" />
                      <span>#{githubLastCommit}</span>
                    </span>
                  </div>
                )}
              </div>
            ...
            {/* Sync Now Spotlight Action Card */}
            <div className="bg-slate-900/90 border border-cyan-500/30 rounded-xl p-5 space-y-4 shadow-lg">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="space-y-1">
                  <div className="flex items-center space-x-2">
                    <h4 className="font-bold text-sm text-white uppercase tracking-wide flex items-center gap-2">
                      <span>Repository Update & Codebase Sync</span>
                      <span className="text-xxs font-black bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 px-2 py-0.5 rounded">
                        /api/settings/sync-github
                      </span>
                    </h4>
                  </div>
                  <p className="text-slate-300 text-xs leading-normal">
                    Clicking <strong>Sync Now</strong> contacts the repository endpoint, pulls the latest commit from branch <code className="bg-slate-950 px-1.5 py-0.5 rounded text-cyan-300 font-mono">{githubBranch || 'main'}</code>, and builds updated assets for the workspace.
                  </p>
                  {githubLastSyncDate && (
                    <div className="text-xxs text-slate-400 pt-1 flex items-center gap-1.5">
                      <CheckCircle2 className="w-3 h-3 text-emerald-400" />
                      <span>Last successful synchronization: {new Date(githubLastSyncDate).toLocaleString()}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {githubRepoUrl && (
                    <a
                      ...
                      className="px-3.5 py-3 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold rounded-xl flex items-center space-x-1.5 transition border border-slate-700"
                    >
                  ...
                  <button
                    ...
                    className="px-6 py-3 bg-gradient-to-r from-cyan-600 via-blue-600 to-indigo-600 hover:from-cyan-500 hover:to-indigo-500 active:from-cyan-700 active:to-indigo-700 text-white text-xs font-black uppercase tracking-wider rounded-xl flex items-center justify-center space-x-2.5 shadow-lg shadow-cyan-950/80 transition cursor-pointer shrink-0 disabled:opacity-50"
                  >
                    <RefreshCw className={`w-4 h-4 text-white ${isGithubSyncing ? 'animate-spin' : ''}`} />
                    <span>{isGithubSyncing ? 'Synchronizing Repository...' : 'Sync Now'}</span>
                  </button>
                </div>
              </div>

              {/* Animated Progress Bar */}
              {(isGithubSyncing || githubSyncProgress > 0) && (
                <div className="space-y-1.5 pt-2">
                  <div className="flex justify-between text-xxs font-bold text-slate-300">
                    <span className="flex items-center gap-1.5 text-cyan-300">
                      <RefreshCw className={`w-3 h-3 ${isGithubSyncing ? 'animate-spin' : ''}`} />
                      <span>{isGithubSyncing ? 'Fetching repository commits & validating modules...' : 'Synchronization complete'}</span>
                    </span>
                    <span className="font-mono text-cyan-400">{githubSyncProgress}%</span>
                  </div>
                  <div className="w-full bg-slate-950 rounded-full h-2.5 overflow-hidden border border-slate-800">
                    <div
                      className="bg-gradient-to-r from-cyan-500 via-blue-500 to-indigo-500 h-2.5 rounded-full transition-all duration-300 shadow-sm"
                      style={{ width: `${githubSyncProgress}%` }}
                    ></div>
                  </div>
                </div>
              )}

              {/* Console Logs Terminal Output Box */}
              {githubSyncLogs.length > 0 && (
                <div className="bg-slate-950 rounded-xl border border-slate-800 p-4 space-y-2 font-mono text-xs shadow-inner">
                  <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                    <div className="flex items-center space-x-2 text-slate-400 text-xxs uppercase tracking-wider font-sans font-bold">
                      <Terminal className="w-3.5 h-3.5 text-cyan-400" />
                      <span>Live Sync Terminal Output</span>
                    </div>
                    <span className="text-xxs text-slate-500">{githubSyncLogs.length} events logged</span>
                  </div>
                      ...
                      <div
                        ...
                        className={`text-xxs font-mono ${
                          log.includes('FAILED') || log.includes('ERROR')
                            ? 'text-rose-400 font-bold bg-rose-950/40 p-1 rounded'
          ...
          {/* Form Card: Configuration Settings */}
          <form onSubmit={handleSaveGithubSettings} className="bg-white rounded-xl border border-slate-200 shadow-xs p-6 space-y-6">
            <div className="border-b border-slate-200 pb-4 flex items-center justify-between">
              <div>
                <h4 className="font-bold text-sm text-slate-900 flex items-center gap-2">
                  <Code2 className="w-4 h-4 text-blue-600" />
                  <span>GitHub Repository Credentials & Target Parameters</span>
                </h4>
                <p className="text-slate-500 text-xs mt-0.5">
                  Configure repository URL, branch, and automated background sync options.
                </p>
              </div>

              <button
                type="submit"
                className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-lg flex items-center space-x-1.5 shadow-xs transition cursor-pointer"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="md:col-span-2 space-y-1.5">
                <label className="text-xs font-bold text-slate-700 flex items-center justify-between">
                  ...
                  <span className="text-xxs text-slate-400 font-normal">e.g. https://github.com/organization/nhc-emr-clinic</span>
                </label>
                <div className="relative">
                  <input
                    ...
                    className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-300 rounded-lg text-xs font-mono text-slate-800 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
                </div>
                <p className="text-xxs text-slate-500">
                  Must be a valid HTTPS or SSH GitHub repository address.
                </p>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder="main"
                    className="w-full px-3.5 py-2 bg-slate-50 border border-slate-300 rounded-lg text-xs font-mono text-slate-800 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
                </div>
                <p className="text-xxs text-slate-500">
                  Default branch for production sync (e.g. <code className="bg-slate-100 px-1 rounded text-slate-700">main</code> or <code className="bg-slate-100 px-1 rounded text-slate-700">master</code>).
                </p>
              </div>

              {/* Secret Token / Access Key */}
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-700 flex items-center justify-between">
                  <span>Personal Access Token / Deploy Secret</span>
                  <span className="text-xxs text-slate-400 font-normal">Optional for private repos</span>
                </label>
                <input
                  type="password"
                  value={githubSecretToken}
                  onChange={(e) => setGithubSecretToken(e.target.value)}
                  placeholder="ghp_xxxxxxxxxxxxxxxxxxxx"
                  className="w-full px-3.5 py-2 bg-slate-50 border border-slate-300 rounded-lg text-xs font-mono text-slate-800 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
                <p className="text-xxs text-slate-500">
                  Required if your GitHub repository is set to Private.
                </p>
              </div>
            </div>
            ...
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 flex items-center justify-between">
              <div className="space-y-0.5">
                <span className="text-xs font-bold text-slate-800 block">Automated Daily Background Synchronization</span>
                <span className="text-xxs text-slate-500 block">
                  Automatically check GitHub for release commits every 24 hours and record updates.
                </span>
              </div>
              <label className="relative inline-flex items-center cursor-pointer">
                <input
          ...
          {/* Table: Synchronization Audit Log History */}
          <div className="bg-white rounded-xl border border-slate-200 shadow-xs p-6 space-y-4">
            <div className="flex items-center justify-between border-b border-slate-200 pb-3">
              <div>
                <h4 className="font-bold text-sm text-slate-900 flex items-center gap-2">
                  <GitCommit className="w-4 h-4 text-emerald-600" />
                  <span>Synchronization Audit Log History</span>
                </h4>
                <p className="text-slate-500 text-xs mt-0.5">
                  Recent repository sync executions and commit records.
                </p>
              </div>

              <span className="text-xxs font-bold text-slate-500 bg-slate-100 px-2.5 py-1 rounded-md border border-slate-200">
                Total Logs: {githubSyncHistory.length}
              </span>
            </div>
              ...
              <table className="w-full text-left text-xs">
                <thead className="bg-slate-100 text-slate-700 font-bold border-b border-slate-200">
                  <tr>
                    ...
                    <th className="px-3.5 py-2.5">Branch</th>
                    <th className="px-3.5 py-2.5">Commit Hash</th>
                    <th className="px-3.5 py-2.5">Status</th>
                    <th className="px-3.5 py-2.5">Message / Details</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-200 text-slate-800">
                  {githubSyncHistory.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="px-4 py-6 text-center text-slate-400 italic text-xs">
                        No repository sync actions recorded yet. Click 'Sync Now' above to pull updates.
                      </td>
                    </tr>
                  ) : (
                    githubSyncHistory.map((item, idx) => (
                      <tr key={idx} className="hover:bg-slate-50 transition">
                        <td className="px-3.5 py-2.5 font-mono text-xxs text-slate-600">
                          {new Date(item.timestamp).toLocaleString()}
                        </td>
                        <td className="px-3.5 py-2.5 font-mono text-xxs font-bold text-slate-700">
                          {item.branch}
                        </td>
                        <td className="px-3.5 py-2.5 font-mono text-xxs text-cyan-700 font-bold">
                          #{item.commitHash}
                        </td>
